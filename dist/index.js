...
`,B=N;var g=class extends T{constructor(){super();this.shadow=this.attachShadow({mode:"open"}),this.shadow.innerHTML=B,this.backgroundEl=this.shadow.querySelector("[part=background]"),this.onBackgroundError=this.onBackgroundError.bind(this)}connectedCallback(){this.backgroundEl.addEventListener("error",this.onBackgroundError);let t=this.getAttribute("background");t&&(this.backgroundEl.src=t)}disconnectedCallback(){this.backgroundEl.removeEventListener("error",this.onBackgroundError)}onBackgroundError(){this.backgroundEl.style.display="none";let t=this.getAttribute("title");this.shadow.querySelector("[part=content]").innerText=t}},O=g;customElements.define("dis-tile",g);var F="https://cd-static.bamgrid.com/dp-117731241344";function V(s){if(s?.programId)return"program";if(s?.seriesId)return"series";if(s?.collectionId)return"collection";l(new Error("Unable to determine source entity for set item"))}function G(s){let t=V(s);return t?{title:s?.text?.title?.full?.[t]?.default?.content||null,tile:s?.image?.tile?.["1.78"]?.[t]?.default?.url||s?.image?.tile?.["1.78"]?.default?.default?.url||null,heroBackground:s?.image?.background?.["1.78"]?.[t]?.default?.url||null,heroVideo:(s?.videoArt||[]).find(e=>e?.purpose==="full_bleed")?.mediaMetadata?.urls?.[0]?.url||null}:null}var H=s=>({title:s?.text?.title?.full?.set?.default?.content||"",items:(s?.items||[]).map(G).filter(Boolean),refId:s?.refId||null});async function M(s){let e=await(await fetch(`${F}/sets/${s}.json`)).json();return H(Object.values(e?.data||{})?.[0])}async function q(){return((await(await fetch(`${F}/home.json`)).json())?.data?.StandardCollection?.containers||[]).map(({set:e}={})=>e&&H(e)).filter(Boolean)}var Q=`
  <div part="track">
    <slot></slot>
  </div>

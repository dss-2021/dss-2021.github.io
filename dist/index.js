...
`,I=W;var p=class extends HTMLElement{constructor(){super();let t=this.attachShadow({mode:"closed"});t.innerHTML=I}hide(){this.style.opacity=0}show(){this.style.opacity=1}},it=p;customElements.define("dis-spinner",p);var $=`
  <slot></slot>

  <dis-spinner></dis-spinner>
`,k=$;var f=class extends HTMLElement{constructor(){super();this.onSlotChange=this.onSlotChange.bind(this);let t=this.attachShadow({mode:"closed"});t.innerHTML=k,this.slotEl=t.querySelector("slot"),this.spinnerEl=t.querySelector("dis-spinner")}connectedCallback(){this.slotEl.addEventListener("slotchange",this.onSlotChange),this.loadScreen().catch(l)}disconnectedCallback(){this.slotEl.removeEventListener("slotchange",this.onSlotChange),this.unloadScreen().catch(l)}onSlotChange(){this.children.length?this.spinnerEl.hide():this.spinnerEl.show()}async loadScreen(){}async unloadScreen(){}},y=f;customElements.define("dis-screen",f);var r={Up:"Up",Down:"Down",Left:"Left",Right:"Right",Select:"Select",Back:"Back"},a={Input:"controlinput"},S=s=>new CustomEvent(a.Input,{bubbles:!0,detail:{input:s}});var c={Focus:"focus"},d,D={ArrowUp:r.Up,ArrowDown:r.Down,ArrowLeft:r.Left,ArrowRight:r.Right,Enter:r.Select,Escape:r.Back};document.body.addEventListener("keyup",s=>{let t=D[s.key];d&&t&&d.dispatchEvent(S(t))});function L(s){d?.removeAttribute("focus"),d=s,d.setAttribute("focus",""),d.dispatchEvent(new CustomEvent(c.Focus,{bubbles:!0}))}var V=`
  <div part="title"></div>
  <div part="track">
